...
class Cam():
    ...
    def run(self):
        ...
        while True:# not self.thread_cancelled:
            #try:
            print("^^^^^^^^^^^^^^^^^")
            shapes = 0
            red = 0
            ...
            if a!=-1 and b!=-1:
                ...
                edge = cv2.Canny(gray, 1500, 1500, apertureSize=5)
...
if __name__ == "__main__":
    ...
    #url = 'http://10.14.194.118:5000/video_feed' #pi
    url = 'http://192.168.1.5:5000/video_feed' #pi
    cam = Cam(url)
    cam.start()
